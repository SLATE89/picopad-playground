...
uint8_t gbRomRead(struct gb_s *gb, const uint_fast32_t addr) {
    ...
    RomBankActual = addr / sizeof(romBank0);

    if (RomBankActual != RomBankLast){
            RomBankLast = RomBankActual;
            FileSeek(&ROMFile, (RomBankActual*sizeof(romBank0)));
            FileRead(&ROMFile, &romBank0, sizeof(romBank0)); 
            return romBank0[addr - RomBankActual*sizeof(romBank0)];
    } else{
        return romBank0[addr];
    }
}

// GameBoy Read Init romBank0
void gbRomInit(){
    u32 status;
    //if (!FileOpen(&ROMFile, "rom.gb")) {
    //    printf("Cannot open file! - ROM init");
    //} else {
        RomBankLast = 0;
        status = FileRead(&ROMFile, &romBank0, sizeof(romBank0));
        //FileClose(&ROMFile);
    //}
    /*
...
void read_cart_ram_file(struct gb_s *gb) {
	...
	if(save_size>0) {
        if (!FileOpen(&SaveFile, filename)) {
            printf("Cannot open file! - Read saves");
        } else {
            FileRead(&SaveFile, &gameRam, save_size);
        }
...
void write_cart_ram_file(struct gb_s *gb) {
	...
	if(save_size>0) {
        if (!FileOpen(&SaveFile, filename)) {
            if(!FileCreate(&SaveFile, filename)){
                printf("Cannot create file!");
            } else {
                FileWrite(&SaveFile, &gameRam, save_size);
            }
            printf("Cannot open file! - Write saves");
        } else {
            FileWrite(&SaveFile, &gameRam, save_size);
        }
...
int main() {
    ...
    //  SD card ROM file structure init
    FileInit(&ROMFile); // initialize file structure of preview file

    if (!FileOpen(&ROMFile, "rom.gb")) {
        printf("Cannot open file! - ROM reading");
    } else{
        // ROM init
        gbRomInit();
    }
    ...
    while (true) {
        ...
        //  SD card save file structure init
        FileInit(&SaveFile); // initialize file structure of preview file
